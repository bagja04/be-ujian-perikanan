...
func JwtExamProtectNes() fiber.Handler {
	return func(c *fiber.Ctx) error {
		// Get Authorization header value
		authHeader := c.Get("Authorization")
		if authHeader == "" {
		...
		// Extract data from claims and add it to the context
		c.Locals("bagian", claims["bagian"].(string))
		c.Locals("typesUjian", claims["typesUjian"].(string))
		c.Locals("namaFungsi", claims["namaFungsi"].(string))
		c.Locals("CodeAkses", claims["CodeAkses"].(string))

		c.Locals("id_bagian", int(claims["id_bagian"].(float64)))
		c.Locals("id_users_ujian", int(claims["id_users_ujian"].(float64)))
